...
time_of_day <- c("00:00 to 01:00","01:00 to 02:00","02:00 to 03:00","03:00 to 04:00",
                 ...
                 "08:00 to 09:00",
                 "09:00 to 10:00","10:00 to 11:00","11:00 to 12:00","12:00 to 13:00",
...
plot_dat <- data.frame()
a <- data.frame(freq, f.cont,time_of_day=rep(time_of_day[7], each=length(f.cont)))
plot_dat <- rbind(a, plot_dat)


g1 <- ggplot(plot_dat, aes(x =time_of_day, y=freq)) +
  geom_tile(aes(fill = f.cont)) +
  #geom_text(aes(label = round(f.cont, 3))) +
  scale_fill_continuous(low = "violetred", high = "aquamarine")+
  theme_bw()


# Create a matrix necessary to visualize the data
data_mat <- plot_dat %>%
  # Convert long-form to wide-form
  spread(key = freq, value = f.cont) %>%
  as.data.frame %>%
  ...
  column_to_rownames(var = "time_of_day") %>%
  as.matrix
