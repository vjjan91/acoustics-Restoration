...
```{r}
# overall space use
glmm_allRestoration <- glmer(roundSpaceuse ~ Restoration.type + (1|siteCode) + (1|visit), data = modelDataAll, family = poisson(link=log))
...
# dawn chorus
glmm_dawnRestoration <- glmer(roundSpaceuse ~ Restoration.type + (1|siteCode) + (1|visit), data = modelDataDawn, family = poisson(link=log))
...
# dusk chorus
glmm_duskRestoration <- glmer(roundSpaceuse ~ Restoration.type + (1|siteCode) + (1|visit), data = modelDataDusk, family = poisson(link=log))
...
totSpaceUse$Restoration.type <- factor(totSpaceUse$Restoration.type, levels = c("Benchmark", "Active", "Passive"))
...
# reordering factors for plotting
totSpaceUseDawn$Restoration.type <- factor(totSpaceUseDawn$Restoration.type, levels = c("Benchmark", "Active", "Passive"))
...
# reordering factors for plotting
totSpaceUseDusk$Restoration.type <- factor(totSpaceUseDusk$Restoration.type, levels = c("Benchmark", "Active", "Passive"))
...
Analysis of ~4,128 hours of acoustic data across treatment types (for the 43 sites) revealed significant differences in ASU between treatment types (Tukey HSD P < 0.05). ASU was significantly different across BM-AR, BM-NR and AR-NR sites, with the highest estimate in BM sites (mean ± SD: 413 ± 103), followed by AR sites (mean ± SD: 188 ± 78) and NR sites (mean ± SD: 182 ± 66). Mean and standard deviation were based on pooled values of ASU across 24 hours for every site-day combination. In the above figure, BM = undisturbed benchmark rainforest sites, AR = Actively restored forest sites and NR = Naturally regenerating forest sites.](figs/fig_04.png)
...
```{r}
# overall space use
glmm_allVeg<- glmer(roundSpaceuse ~ PC1 + PC2 + (1|siteCode) + (1|visit), data = modelDataAll, family = poisson(link = log))

summary(glmm_allVeg)
plot_model(glmm_allVeg, type="pred", terms=c("PC1","PC2"))
report::report(glmm_allVeg)

# There is a significant negative effect of PC1 and PC2 on acoustic space use

# dawn chorus
glmm_dawnVeg<- glmer(roundSpaceuse ~ PC1 + PC2 + (1|siteCode) + (1|visit), data = modelDataDawn, family = poisson(link = log))

summary(glmm_dawnVeg)
plot_model(glmm_dawnVeg, type="pred", terms=c("PC2","PC1"))
report::report(glmm_dawnVeg)

# There is a significant effect of PC1 and PC2 on dawn acoustic space use and a highly significant effect of PC2 on dawn acoustic space use

# dawn chorus
glmm_duskVeg<- glmer(roundSpaceuse ~ PC1 + PC2 + (1|siteCode) + (1|visit), data = modelDataDusk, family = poisson(link = log))
