...
fig_nmds <- ggplot(data=nmdsScores) + 
  ...
  theme(axis.title = element_text(family="Century Gothic",
   ...
   annotate(geom = "text", x = max(nmdsScores$NMDS1), y = max(nmdsScores$NMDS2),  label = "Stress = 0.007", hjust = 1, family = "Century Gothic", size=5)
