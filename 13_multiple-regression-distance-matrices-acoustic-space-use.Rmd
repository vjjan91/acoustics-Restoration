...
nmdsScores <- read.csv("data/nmds-acousticSpaceUse.csv")
spaceUseMatrix <- dist(nmdsScores[,1:2], method = "euclidean", diag = TRUE, upper = TRUE, p = 2)
spaceUseMatrix <- as.matrix(spaceUseMatrix)
```
...
Running mantel's tests on the dissimilarity matrices of space use (nmds scores), vegetation (pca scores) and flora abundance (nmds scores)
```{r}
mantelSpacePca <- vegan::mantel(spaceUseMatrix, vegPcaScores)
mantelSpaceFlora <- vegan::mantel(spaceUseMatrix, floraMatrix)

# Mantel's and partial mantel's tests suggest no strong relationship

vegan::mantel.partial(spaceUseMatrix, vegPcaScores, distMatrix)
vegan::mantel.partial(spaceUseMatrix, vegPcaScores, floraMatrix)
```
...
spaceUseVector <- upper.tri.remove(spaceUseMatrix)
diag(spaceUseVector) <- NA
spaceUseVector <- cbind(c(spaceUseVector)) %>% na.omit()
...
mrmData <- bind_cols (spaceUseVector, floraVector, pcaVector, distanceVector) %>% rename (spaceUse = ...1, flora = ...2, structure = ...3, distance = ...4)

MRM(dist(spaceUse) ~ dist(flora) + dist(structure) + dist(distance), data = mrmData, method = "linear", nperm = 1000)

# The above result suggests that space use compositional dissimilarity is not signficantly related to dissimilarity in habitat structure, geographic distance and florstics (Please note that the overall R2 values are very low)
```
...
```{r}
hier.part(mrmData$spaceUse, mrmData[2:4], family = "gaussian", gof = "Rsqu", barplot = TRUE)
```
