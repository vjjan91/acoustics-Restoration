...
Running a GLMM that compares avian species and vegetation specific
```{r}
# To run a GLMM, we need to first extract necessary predictors that will be use in the mixed-effects model. 

# 1. Calculate the overall number of detections for each site across X days of data (translates to 16min to 48min of data per site; each detection corresponding to a temporal unit of 10seconds). Here, we 
...
nSites_Days <- data %>%
 ...
 filt_data %>% 
  ...
  summarise (
    ...
    visits = n_distinct(Site_ID, Date))
