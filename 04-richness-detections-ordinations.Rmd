...
sum_DetectionsSite <- nDetections_Site %>%
  ...
  select(Site, Restoration.Type, sumDetections)
...
# Next steps: 
# 1. Include species trait data to check if number of detections varies by treatment type as a function of whether a species is a rainforest specialist vs. open-country specialist.
# To visualize the above, imagine two-box plots for each treatment type (rainforest specialist and open country)

# Potential Idea: Pivot species codes to long and then match by species-trait-dat (RF/OC)
# trial <- nDetections_Site %>%
...
richness <- nDetections_Site  %>% 
  ...
  select(Site, Restoration.Type, richness)
