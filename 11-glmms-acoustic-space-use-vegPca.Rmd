...
# Calculating total space use across all frequency bins and times of day: 128*24

totSpaceUse <- siteWiseAsu %>%
  group_by(Site, Restoration.type) %>%
  summarise(totSpaceuse = sum(f.cont.sum)) 

# Calculating total space use for dawn chorus alone

totSpaceUseDawn <- dawnChorusAsu %>%
  group_by(Site, Restoration.type) %>%
  summarise(totSpaceuse = sum(f.cont.sum)) 

# Calculating total space use for dusk chorus alone
...
modelDataAll <- vegPcaScores %>% 
  ...
  add_column (spaceUse = totSpaceUse$totSpaceuse,
...
modelDataDawn <- vegPcaScores %>% 
  ...
  rename(Restoration.type = Site_type) %>%
  mutate(across(Restoration.type, factor))  %>%
  add_column (spaceUse = totSpaceUseDawn$totSpaceuse,
              plantingYear = vegData$plantingYear) %>%
  mutate(siteCode = str_extract(Site, pattern = "\\w+\\d+")) %>%
  mutate(siteCode = factor(siteCode))
...
modelDataDusk <- vegPcaScores %>% 
  ...
  rename(Restoration.type = Site_type) %>%
  mutate(across(Restoration.type, factor))  %>%
  add_column (spaceUse = totSpaceUseDusk$totSpaceuse,
              plantingYear = vegData$plantingYear) %>%
  mutate(siteCode = str_extract(Site, pattern = "\\w+\\d+")) %>%
  mutate(siteCode = factor(siteCode)) 
```
...
```{r}
# overall space use
glm_totSpaceUse <- glm(spaceUse ~ Restoration.type, data = modelDataAll, family = gaussian)
summary(glm_totSpaceUse)
