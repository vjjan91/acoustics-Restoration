...
# For open country birds, there is a significant difference in first-order jacknife estimates between each treatment type
```
...
# Results above suggest PC2 is significantly negatively associated with richness of rainforest birds. A similar result is seen below for open-country bird species richness and PC1
...
allRestored <- modelDataAll %>%
  ...
  mutate(yearSinceRestoration = (2021-year))

glmAll <- glm(richness ~ yearSinceRestoration, data = allRestored, family = poisson(link = log))
summary(glmAll)

# rainforest bird richness
rainRestored <- modelData_rainForest %>%
  filter(Restoration.type=="Active") %>%
  mutate(yearSinceRestoration = (2021-year))

glmRain <- glm(roundjk ~ yearSinceRestoration, data = rainRestored, family = poisson(link = log))
summary(glmRain)

# open-country richness
openRestored <- modelData_openCountry %>%
  filter(Restoration.type=="Active") %>%
  mutate(yearSinceRestoration = (2021-year))

glmOpen <- glm(roundjk ~ yearSinceRestoration, data = openRestored, family = poisson(link = log))
summary(glmOpen)
