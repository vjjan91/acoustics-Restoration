...
# With the above analysis, we note that the highest increase in r2 and biggest drop in stress as at 2 dimensions (from meanR2 = 0.03 to 0.62 at 2 dimensions and drop in mean stress from 0.55 to 0.24 at 2 dimensions). However, if stress is high, we should reposition the points in 2 dimensions in the direction of decreasing stress, and repeat until stress is below some threshold.**A good rule of thumb: stress < 0.05 provides an excellent representation in reduced dimensions, < 0.1 is great, < 0.2 is good/ok, and stress < 0.3 provides a poor representation.** To reiterate: high stress is bad, low stress is good!
```
...
# Minimum stress is 0.0877689 and r^2 for minimum stress configuration:  0.8393313 
...
# Here, we know that NMDS axis 3, followed by NMDS Axis 1 has the highest explanatory power and we write out the .csv of the same
write.csv(nmdsBrayCurtis$conf[[1]], "data/nmdsBrayCurtis.csv", row.names = F)
```
...
fig_nmds <- ggplot(data=nmdsPower) + 
  stat_ellipse(aes(x=X1,y=X3,colour=Restoration.Type),level = 0.50) +
  geom_point(aes(x=X1,y=X3,shape=Restoration.Type,colour=Restoration.Type),size=4) + 
  theme_bw() +
...
groups <- nmdsPower$Restoration.Type
